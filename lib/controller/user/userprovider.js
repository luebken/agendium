...
UserProvider.prototype.delete = function(id, callback) {
    this.getCollection(function(error, users_collection) {
        if (error) console.log(error)
            ...
            users_collection.remove({'_id':ObjectID.createFromHexString(id)}, function(err, data) {
...
UserProvider.prototype.updateShownIntro = function(email, oldhashedpassword, shownintro, callback) {
    this.getCollection(function(error, users_collection) {
        if (error) console.log(error)
        else {
            var criteria = {'email':email, 'hashedpassword':oldhashedpassword};
            var doc = {$set:{'shoswnintro':shownintro}};
            users_collection.update(criteria, doc, {'safe':true}, function(err, inserted_data) {
                callback(err, inserted_data);
            });
        }
    });
