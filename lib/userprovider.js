    ...
    this.getCollection(function(error, users_collection) {
      if( error ) console.log(error)
        ...
        users_collection.findOne({email: email2, password: password2}, function(error, result) {
            console.log('users_collection.findOne Error')
            console.log(error);
            console.log('users_collection.findOne result')
            console.log(result);
...
UserProvider.prototype.updatePassword = function(email, oldpassword, newpassword, callback) {
    this.getCollection(function(error, users_collection) {
      if( error ) console.log(error)
          ...
          var spec = {'email':email, 'password':oldpassword};
          var doc = {$set:{'password':newpassword}};
